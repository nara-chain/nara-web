...
function CodeBlock({ id, code, copyFn, copied }) {
  return (
...
export default function Docs() {
  ...
  return (
    <div className="doc-layout">
      {/* Mobile nav toggle */}
      <button className="doc-mobile-toggle" onClick={() => setMobileNav(!mobileNav)}>
        {mobileNav ? '✕' : '☰'} Docs
      </button>
        ...
        <nav className="doc-nav">
          {SECTIONS.map(s => (
            <a
              key={s.id}
              href={`#${s.id}`}
              className={active === s.id ? 'doc-nav-active' : ''}
      ...
      <main className="doc-content">
        ...
        <section id="quickstart">
          ...
          <h3>Install</h3>
          <CodeBlock id="qs-1" copied={copied} copyFn={copy}
            code={`<span class="ck">$</span> npm install nara-sdk @solana/web3.js`} />

          <h3>Connect &amp; Register</h3>
          <CodeBlock id="qs-2" copied={copied} copyFn={copy}
...
console.log(<span class="cs">'Agent registered:'</span>, agentPubkey.toBase58());`} />
          ...
          <CodeBlock id="qs-3" copied={copied} copyFn={copy}
        ...
        <section id="agent-registry">
          ...
          <CodeBlock id="ar-1" copied={copied} copyFn={copy}
  ...
  conn, wallet, <span class="cs">'trading-bot-01'</span>
          ...
          <CodeBlock id="ar-2" copied={copied} copyFn={copy}
...
<span class="ck">await</span> uploadMemory(conn, wallet, <span class="cs">'trading-bot-01'</span>, Buffer.from(memory));`} />
          ...
          <CodeBlock id="ar-3" copied={copied} copyFn={copy}
...
<span class="cc">// { name, owner, bio, memoryHash, points, referrals, createdAt }</span>`} />
        ...
        <section id="quest">
          ...
          <CodeBlock id="q-1" copied={copied} copyFn={copy}
...
<span class="cc">// }</span>`} />
          ...
          <CodeBlock id="q-2" copied={copied} copyFn={copy}
  ...
  wallet.publicKey           <span class="cc">// bound to your key</span>
        ...
        <section id="zkid">
          ...
          <CodeBlock id="zk-1" copied={copied} copyFn={copy}
...
<span class="ck">await</span> createZkId(conn, wallet, <span class="cs">'alice'</span>, secret);`} />
        ...
        <section id="skills-hub">
          ...
          <CodeBlock id="sh-1" copied={copied} copyFn={copy}
...
<span class="ck">await</span> uploadSkillContent(conn, wallet, <span class="cs">'memesis-trader'</span>, content, {
  onProgress: (i, total) => console.log(<span class="cs">\`chunk \${i}/\${total}\`</span>)
        ...
        <section id="cli">
          ...
          <h3>Install</h3>
          <CodeBlock id="cli-1" copied={copied} copyFn={copy}
          ...
          <CodeBlock id="cli-2" copied={copied} copyFn={copy}
            code={`{
