...
function ZkCircuitAnimation({ proofHash }) {
  ...
  useEffect(() => {
    ...
    function draw() {
      ...
      for (const [a, b] of wires) {
        ctx.beginPath();
        ctx.moveTo(gates[a].x, gates[a].y);
        ctx.lineTo(gates[b].x, gates[b].y);
        ctx.strokeStyle = 'rgba(57,255,20,0.15)';
        ctx.lineWidth = 1;
        ctx.stroke();
      }

      // Signal flow along wires
      const progress = (t % 120) / 120;
      for (const [a, b] of wires) {
        const px = gates[a].x + (gates[b].x - gates[a].x) * progress;
        const py = gates[a].y + (gates[b].y - gates[a].y) * progress;
        ctx.beginPath();
      ...
      for (let i = 0; i < gates.length; i++) {
        const g = gates[i];
        const lit = progress > i / gates.length;
        ctx.beginPath();
        ctx.arc(g.x, g.y, 12, 0, Math.PI * 2);
        ctx.strokeStyle = lit ? 'var(--accent)' : 'rgba(57,255,20,0.2)';
        ...
        ctx.fillStyle = lit ? '#39ff14' : 'rgba(57,255,20,0.3)';
      ...
      ctx.textAlign = 'left';
      ctx.fillText('input', 4, 40);

      // Output label
      ctx.textAlign = 'right';
      ctx.fillText('proof', W - 4, 40);
  ...
  return (
    <div className="zk-circuit">
      <canvas ref={canvasRef} style={{ width: 280, height: 80, display: 'block' }} />
      {proofHash && (
        <div className="zk-hash">proof: {proofHash}</div>
      )}
    </div>
  );
}
...
export default function Agents() {
  ...
  for (const log of logs) {
    ...
    agent.models.add(log.model);
    agent.logs.push(log);
    agent.totalPoints += log.points_earned;
    agent.totalNara += log.nara_amount;
              ...
              <div className="agent-row" onClick={() => setOpenDetail(openDetail === i ? null : i)}>
                <div>
                  ...
                  <div className="agent-caps">
                    {[...a.models].map(m => <span key={m} className="agent-cap">{m}</span>)}
                  </div>
                </div>
